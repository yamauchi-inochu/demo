...
　課題形式で利用する場合は、[実践ページ](../実習/実習ページ/GIS入門.md)をご利用ください。
...
## データの読み込み
　GISで読み込むデータは、多くく分けてベクトルデータとラスタデータがある。QGISでは、以下のようにデータによって読み込む手法が変わる。
...
## 属性テーブル
　ベクトルデータは、位置情報のほかに属性情報を保持している。GISでは、属性情報を用いたデータの切り出しや解析などができる。
...
## プロパティ
　QGISでは、プロパティからデータの情報やスタイルの変更ができる。ポイントでは形や大きさ、ラインでは線の形や太さ、ポリゴンでは塗りつぶし色や透過性を変更することができる。また、属性情報でスタイルを分けることもできる。しかし実際には、データそのものの形状や属性等が改変されるわけではない。
...
## データの保存形式
　GISでは、Shapefile（シェープファイル）と呼ばれるベクトルデータを用いることが多い。しかし、使用するソフトウェアやアプリケーションによっては、別の形式のファイルを使用する。QGISでは、以下ののように任意のファイル形式で保存を行う。
...
## プラグイン
　QGISでは、オフィシャルプラグインリポジトリ等から、様々な拡張機能をプラグインとしてインストールできる。プラグインをインストールすることにより、様々な処理が可能になる。
...
### タイルレイヤプラグイン
　プラグインの管理とインストールからTileLayerPluginをインストールし、地理院タイルをまとめた[gsi-tiles.tsv](https://github.com/yamauchi-inochu/demo/raw/master/GISオープン教材/実習/sample/gsi-tiles.zip)をダウンロードし、メニューバーのweb>タイルレイヤプラグインから、プラグインを起動し、.tsvが入っているファイルを指定するとタイルが表示できる。地理院タイルを使用する際は、[地理院タイルの利用規約](http://maps.gsi.go.jp/help/use.html)に従ってください。

※タイルレイヤプラグインを使う際は、先にデータを読み込んだ状態でプラグインからタイルを読み込む。  
※本教材で用意したtsvファイルは、Minoru Akagi(minoura)氏 が、[GitHub Gist](https://gist.github.com/minorua/7654132) で公開している.tsvを加工し作成したものです。  

[▲メニューへもどる]  

## 地図のレイアウト
　QGISでは作成したデータに凡例や縮尺を追加した地図がレイアウトできる。
